...
type ProgressProps = { months: ProgramMonth[]; logs: Record<string, { steps: number; water: number; sleep: number; calories: number; protein: number; weight?: number; completed: string[]; submitted?: boolean }> };
export function ProgressView({ months, logs }: ProgressProps) {
  const active = months[months.length-1]; const isSubmitted = (day: number) => !!logs[`${active.id}-${day}`]?.submitted; const points = active.days.map(d => ({ day:d.day, steps:isSubmitted(d.day) ? logs[`${active.id}-${d.day}`]?.steps||0 : 0, weight:isSubmitted(d.day) ? logs[`${active.id}-${d.day}`]?.weight : undefined })).filter(p=>p.steps||p.weight);
  const avgSteps = points.length ? Math.round(points.reduce((a,p)=>a+p.steps,0)/points.length) : 0;
  const completed = active.days.filter(d => isSubmitted(d.day) && (logs[`${active.id}-${d.day}`]?.completed.length||0)>=d.exercises.length).length;
  const max = Math.max(8000,...points.map(p=>p.steps));
  return <><div className="eyebrow">TRENDS · WEEKLY CHECKPOINTS</div><div className="page-heading"><div><h1>Read the trend, not the noise.</h1><p>Only submitted daily logs appear here.</p></div></div><div className="progress-stats"><article><Trophy/><span>WORKOUTS</span><strong>{completed}<small>/30</small></strong></article><article><Footprints/><span>AVG STEPS</span><strong>{avgSteps.toLocaleString()}</strong></article><article><Droplets/><span>WATER LOGGED</span><strong>{points.filter(p=>logs[`${active.id}-${p.day}`]?.water).length}<small> days</small></strong></article><article><Moon/><span>7H+ SLEEP</span><strong>{active.days.filter(d=>isSubmitted(d.day) && (logs[`${active.id}-${d.day}`]?.sleep||0)>=7).length}<small> days</small></strong></article></div><section className="chart-card"><div className="section-title"><h2>Daily steps</h2><span>Submitted logs only</span></div><div className="bar-chart">{active.days.map(d => { const value=isSubmitted(d.day) ? logs[`${active.id}-${d.day}`]?.steps||0 : 0; return <div key={d.day} title={`Day ${d.day}: ${value} steps`}><span style={{height:`${Math.max(2,(value/max)*100)}%`}}/><small>{d.day%5===0?d.day:""}</small></div>})}</div></section><section className="checkpoint-grid">{[7,14,21,30].map(day=><article key={day}><span>DAY {day}</span><h3>{day===30?"Month review":"Weekly checkpoint"}</h3><p>Review submitted weight, waist, adherence, strength notes, sleep and safety notes.</p><button onClick={()=>window.print()}>Print review</button></article>)}</section></>;
}
...
export type PlanExercise = { id: string; name: string; sets: number; repRange: string; rest: string; rir: string; cue: string; mistakes: string; easier: string; harder: string; equipment: string; replacement?: string; safeNote?: string; image?: string };
const planTemplates = [
...
const exerciseImages: Record<string, string> = {
  "push-up":"/exercises/pose-body-1-1.png", "incline-push-up":"/exercises/pose-body-1-2.png", "diamond-push-up":"/exercises/pose-body-1-3.png", "pike-push-up":"/exercises/pose-body-1-4.png",
  "dumbbell-press":"/exercises/pose-dumbbell-1-1.png", "flat-db-press":"/exercises/pose-dumbbell-1-2.png", "incline-db-press":"/exercises/pose-dumbbell-1-3.png", "close-grip-db-press":"/exercises/pose-body-1-3.png", "dumbbell-fly":"/exercises/pose-dumbbell-1-4.png", "incline-db-fly":"/exercises/pose-dumbbell-1-4.png",
  "shoulder-press":"/exercises/pose-dumbbell-2-3.png", "lateral-raise":"/exercises/pose-dumbbell-2-4.png", "front-raise":"/exercises/pose-dumbbell-3-1.png", "rear-delt-fly":"/exercises/pose-dumbbell-2-2.png",
  "row":"/exercises/pose-dumbbell-2-1.png", "reverse-fly":"/exercises/pose-dumbbell-2-2.png", "curl":"/exercises/pose-dumbbell-3-2.png", "hammer-curl":"/exercises/pose-dumbbell-3-3.png", "triceps-extension":"/exercises/pose-dumbbell-3-4.png", "overhead-db-extension":"/exercises/pose-dumbbell-3-4.png", "triceps-kickback":"/exercises/pose-dumbbell-2-1.png",
  "squat":"/exercises/pose-body-2-1.png", "bodyweight-squat":"/exercises/pose-body-2-1.png", "rdl":"/exercises/pose-dumbbell-2-1.png", "reverse-lunge":"/exercises/pose-body-2-2.png", "glute-bridge":"/exercises/pose-body-3-2.png", "calf-raise":"/exercises/pose-body-3-3.png",
  "plank":"/exercises/pose-body-2-3.png", "dead-bug":"/exercises/pose-body-3-2.png", "superman":"/exercises/pose-body-2-4.png", "bird-dog":"/exercises/pose-body-2-4.png", "mountain-climber":"/exercises/pose-body-3-1.png", "bear-crawl":"/exercises/pose-body-3-1.png", "high-knees":"/exercises/pose-body-3-4.png", "burpee":"/exercises/pose-body-3-1.png",
  "cobra-pose":"/exercises/pose-mobility-1-1.png", "child-pose":"/exercises/pose-mobility-1-2.png", "cat-cow":"/exercises/pose-mobility-1-3.png", "worlds-greatest-stretch":"/exercises/pose-mobility-1-4.png", "butterfly-stretch":"/exercises/pose-mobility-2-1.png", "hip-rotation":"/exercises/pose-mobility-2-2.png", "open-book":"/exercises/pose-mobility-2-3.png", "wall-slide":"/exercises/pose-mobility-2-4.png", "chin-tuck":"/exercises/pose-mobility-3-1.png"
};
const fallbackExerciseImage = (focuses: string[]) => focuses.includes("Stretching") ? "/exercises/pose-mobility-3-2.png" : focuses.includes("Core") || focuses.includes("Cardio") ? "/exercises/pose-body-3-1.png" : focuses.includes("Full lower body") || focuses.includes("Calves") ? "/exercises/pose-body-2-1.png" : focuses.includes("Back") ? "/exercises/pose-dumbbell-2-1.png" : "/exercises/pose-dumbbell-2-3.png";
...
].map(exercise => ({ ...exercise, image: exerciseImages[exercise.id] ?? fallbackExerciseImage(exercise.focuses) }));
