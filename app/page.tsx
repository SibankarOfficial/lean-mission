...
const emptyLog = (): DailyLog => ({ steps: 0, water: 0, sleep: 0, calories: 0, protein: 0, energy: 3, soreness: 1, neck: 0, notes: "", completed: [], sets: {} });
...
function AppContent() {
  ...
  const taskCount = today.exercises.length + 5;
  const doneCount = log.completed.length + [log.steps >= today.stepTarget, log.water >= 2500, log.sleep >= 7, log.calories > 0, log.protein >= 100].filter(Boolean).length;
      ...
      <Tabs value={tab} onValueChange={setTab} className="workspace">
        <aside className="sidebar">
          ...
          <TabsList orientation="vertical" className="nav-list">
            ...
            <TabsTrigger value="safety"><ShieldCheck /> Safety</TabsTrigger>
            <TabsTrigger value="settings"><Settings /> Settings</TabsTrigger>
          </TabsList>
          <div className="safety-toggle"><div><ShieldCheck /><span><b>Cervical Safe</b><small>High-risk moves replaced</small></span></div><Switch checked={state.safeMode} onCheckedChange={v => setState(s => ({ ...s, safeMode: v }))} /></div>
        </aside>

        <section className="content">
          <TabsContent value="dashboard">
            <div className="eyebrow">MONDAY · DAY {today.day}</div>
            <div className="page-heading"><div><h1>Today, made simple.</h1><p>{today.focus}. Aim for calm, clean reps—not exhaustion.</p></div><button className="primary" onClick={() => setTab("today")}>Start today <ChevronRight /></button></div>
            <div className="dashboard-grid">
              <section className="today-card"><div className="card-top"><span className="workout-label"><Dumbbell /> TODAY&apos;S TRAINING</span><span>~38 min</span></div><h2>{today.type}</h2><p>{today.exercises.length} exercises · {today.skillMinutes} min skill prep · {today.recovery}</p><div className="exercise-preview">{today.exercises.slice(0, 3).map((e, i) => <div key={e.id}><span>{String(i + 1).padStart(2, "0")}</span><b>{e.name}</b><small>{e.sets} × {e.repRange}</small></div>)}</div><button onClick={() => setTab("today")}>Open workout <ChevronRight /></button></section>
            ...
            <div className="metrics-grid">
              <Metric icon={<Footprints />} label="Steps" value={log.steps.toLocaleString()} detail={`${today.stepTarget.toLocaleString()} target`} />
              <Metric icon={<Waves />} label="Water" value={`${(log.water / 1000).toFixed(1)} L`} detail="2.5 L editable target" />
              <Metric icon={<Utensils />} label="Nutrition" value={`${log.calories} kcal`} detail={`${log.protein} / 110 g protein`} />
              <Metric icon={<Moon />} label="Sleep" value={`${log.sleep || "—"} hr`} detail="7+ hour recovery target" />
            </div>
          ...
          <TabsContent value="today">
            <div className="eyebrow">DAY {today.day} · {activeMonth.name.toUpperCase()}</div><div className="page-heading"><div><h1>{today.type}</h1><p>{today.focus} · target {today.stepTarget.toLocaleString()} steps</p></div><span className="safe-badge"><ShieldCheck /> Safe Mode {state.safeMode ? "on" : "off"}</span></div>
            <section className="checkin-strip"><label>Weight, kg <input type="number" step=".1" value={log.weight||""} onChange={e=>updateLog({weight:+e.target.value})} placeholder="optional"/></label><label>Sleep, hr <input type="number" step=".5" value={log.sleep||""} onChange={e=>updateLog({sleep:+e.target.value})}/></label><label>Energy, 1–5 <input type="number" min="1" max="5" value={log.energy} onChange={e=>updateLog({energy:+e.target.value})}/></label><label>Soreness, 1–5 <input type="number" min="1" max="5" value={log.soreness} onChange={e=>updateLog({soreness:+e.target.value})}/></label><label>Neck, 0–10 <input type="number" min="0" max="10" value={log.neck} onChange={e=>updateLog({neck:+e.target.value})}/></label></section>
            {(log.neck>=5 || log.energy<=1 || log.soreness>=5) && <aside className="fatigue-warning"><ShieldCheck/><p><b>Regression suggested today.</b> Reduce each exercise by one set, leave 4 reps in reserve, and use the easier variation. Stop if symptoms radiate or neurological signs appear.</p></aside>}
